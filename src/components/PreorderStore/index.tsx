...
const PreorderStore: FunctionComponent<TPreorderStoreProps> = ({
    ...
    return (
        <div className={classes.store_container}>
            ...
            <span className={`${classes.tag} ${locationFilter === location ? classes.tag_active : ""}`} onClick={() => filterLocation(location)}>{location}<span className={classes.close_tag}>x</span></span>
            <span className={classes.tag} onClick={() => filterEdition("normal")}>{texts.preorderStore.normalEdition}</span>
            { hasSpecial &&
                <span className={`${classes.tag} ${editionFilter ? classes.tag_active : ""}`} onClick={() => filterEdition("special")}>{texts.preorderStore.specialEdition}<span className={classes.close_tag}>x</span></span>     
            }
        </div>
    );
}
